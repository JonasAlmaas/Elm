...
namespace elm {
	...
	struct render_command
	{
		...
		inline static void get_viewport(uint32_t *x, uint32_t *y, uint32_t *width, uint32_t *height) { s_api->get_viewport(x, y, width, height); }
		...
		inline static void draw_triangles(uint32_t count, uint32_t offset = 0) { s_api->draw_triangles(count, offset); } 
